...
void ParticleEmitter::ItemAdd(int count, int life, XMFLOAT3 position, Model* model)
{
	...
	{
		//X,Y,Z全て[-5.0,+5.0]でランダムに分布
		this->position_.x = ((float)rand() / RAND_MAX * objMdPos_.x - objMdPos_.x / 2.0f) + position.x;
		this->position_.y = ((float)rand() / RAND_MAX * objMdPos_.y - objMdPos_.y / 2.0f) + position.y - 100;
		this->position_.z = ((float)rand() / RAND_MAX * objMdPos_.z - objMdPos_.z / 2.0f) + position.z;
		//X,Y,Z全て[-0.05,+0.05]でランダムに分布
		velocity_.y = mdVel_;
		//重力に見立ててYのみ[-0.001f,0]でランダムに分布
		...
		if (particleMan_ != nullptr)
		{
			particleMan_->Add(life, this->position_, {}, accel_, startScale_, endScale_, startColor_, endColor_);
		}
